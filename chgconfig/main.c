...
// for 2 icons
static char button_color[2][32];
static RsvgStylePair color_pair [2];
static cairo_t * icon_cr[2];
static cairo_surface_t * icon_surface[2];

static cairo_t * bright_cr;
static cairo_surface_t * bright_surface;
static RsvgHandle * m_bright_svg_handle = NULL;  // svg handle for arrow file
                                                    ...
                                                    int width, int height)
{
    ...
    if(file_name)
    {
        ...
        memset(button_color[0], 0, 32);
        if(GetValueFromEtcFile(file_path, "AppConfigData", "icon0_color", \
                                            button_color[0], 32) == ETC_OK)
        {
            color_pair[0].name = "color";
            color_pair[0].value = button_color[0];
            color_pair[0].important = 0;
        }

        memset(button_color[1], 0, 32);
        if(GetValueFromEtcFile(file_path, "AppConfigData", "icon1_color", \
                                            button_color[1], 32) == ETC_OK)
        {
            color_pair[1].name = "color";
            color_pair[1].value = button_color[1];
            color_pair[1].important = 1;
        }
    }
...
static void loadSVGFromFile(const char* file, int index)
{
    ...
    icon_surface[index] = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, height);
    factor_width = (double)width / (double)dimensions.width;
    factor_height = (double)height / (double)dimensions.height;
    icon_cr[index] = cairo_create(icon_surface[index]);

    cairo_save(icon_cr[index]);
    factor_width = (factor_width > factor_height) ? factor_width : factor_height;
    cairo_scale(icon_cr[index], factor_width, factor_width);
    ...
    cairo_set_source_rgb(icon_cr[index],  r * alpha, g * alpha, b * alpha);
    cairo_paint(icon_cr[index]);
    rsvg_handle_render_cairo_style(handle, icon_cr[index], &color_pair[index], 1);
    cairo_restore(icon_cr[index]);
...
static void paint_svg(HWND hwnd, HDC hdc, int index)
{
    ...
    HDC csdc = create_memdc_from_image_surface(icon_surface[index]);
...
static LRESULT ChgConfigWinProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
        ...
        case MSG_CLOSE:
            if(hmapdc)
                DeleteMemDC(hmapdc);
            cairo_surface_destroy(icon_surface[0]);
            cairo_surface_destroy(icon_surface[1]);
            cairo_destroy(icon_cr[0]);
            cairo_destroy(icon_cr[1]);
